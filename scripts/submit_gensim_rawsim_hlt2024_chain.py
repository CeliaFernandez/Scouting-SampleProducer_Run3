#!/usr/bin/env python3
"""
GEN-SIM -> RAWSIM+HLT2024 launcher for DQCD Run3 2024 production, chained in
a single CRAB job (no intermediate GEN-SIM dataset staged out or published).

Same design as submit_gensim_rawsim_chain.py: GEN-SIM is produced first,
RAWSIM(+HLT) second — two separate cmsDriver-generated cfgs, each run by its
own cmsRun call — but both happen inside ONE CRAB job via
JobType.pluginName='PrivateMC' + JobType.scriptExe=run_chain.sh (shared with
that script). The only differences are the step2 --step list
(DIGI,DATAMIX,L1,DIGI2RAW,HLT:2024v14 instead of DIGI,DATAMIX,L1,DIGI2RAW)
and the GT/release, both of which must match what HLT:2024v14 resolves
against:
  ...
  cmsDriver.py --era Run3_2024 \\
    ...
    --geometry DB:Extended --conditions 140X_mcRun3_2024_realistic_v26 \\
    ...
    --filein file:<gensim>.root --fileout file:<rawsim_hlt>.root --no_exec --mc

IMPORTANT: unlike submit_gensim_rawsim_chain.py (CMSSW_17_0_0_pre2, GT
150X_mcRun3_2024_realistic_v3), both step1 and step2 here run under
CMSSW_14_0_21 with GT 140X_mcRun3_2024_realistic_v26 — the GT the
HLT:2024v14 menu was actually calibrated against, which isn't available
under 17X:
...
Assumes:
  - cmsenv has been sourced inside the CMSSW_14_0_21 area above
...
Usage:
  python3 submit_gensim_rawsim_hlt2024_chain.py [options]
  python3 submit_gensim_rawsim_hlt2024_chain.py --dry-run
  python3 submit_gensim_rawsim_hlt2024_chain.py --filter ScenarioA
  python3 submit_gensim_rawsim_hlt2024_chain.py --filter mpi-1p2
"""

# CMSSW_14_0_21 ships Python 3.9, which doesn't evaluate `str | None`-style
...
REQUIRED_CMSSW_RELEASE = "CMSSW_14_0_21"
...
STEP2_OUTPUT = "rawsim_hlt2024.root"

# Reduced event content for the final GEN-SIM-RAW-HLT2024 output (see
# README.md "Event content - reduced"): keeps only what the scouting HLT
# re-emulation needs instead of the full PREMIXRAW content.
STEP2_OUTPUT_COMMANDS = (
    "process.PREMIXRAWoutput.outputCommands = cms.untracked.vstring("
    '"drop *",'
    '"keep *_*Packer*_*_*",'
...
STEP2_CMSDRIVER_TEMPLATE = (
    ...
    f" --customise_commands '{STEP2_OUTPUT_COMMANDS}'"
    " --python_filename {cfg}"
    f" --fileout file:{STEP2_OUTPUT}"
...
config.General.requestName = '{name}_GENSIM_RAWSIM_HLT2024'
...
RUN_CHAIN_TEMPLATE = """\
#!/bin/bash
# Generated by submit_gensim_rawsim_hlt2024_chain.py -- do not edit by hand.
set -e
...
# Step 2: RAWSIM+HLT2024, reading {step1_output} -> final output.
...
def check_cmssw_release(dry_run: bool) -> None:
    """--step ...,HLT:2024v14 resolves the HLT menu/table shipped in
    REQUIRED_CMSSW_RELEASE, and 140X_mcRun3_2024_realistic_v26 isn't
    available under 17X; running from another release can silently pick up a
    different (incompatible) menu/GT."""
...
def is_already_submitted(work_area: Path, name: str) -> bool:
    """CRAB creates crab_<name>_GENSIM_RAWSIM_HLT2024/ inside workArea after a successful submit."""
    return (work_area / name / f"crab_{name}_GENSIM_RAWSIM_HLT2024").exists()
...
def generate_step2_cfg(work_area: Path, pileup_input: str, dry_run: bool) -> Path:
    """Generate the single DIGI/RAW+HLT cfg shared by every sample (mirrors
...
def write_crab_config(name: str, step1_cfg: Path, step2_cfg: Path, run_chain: Path,
    ...
    crab_cfg = job_dir / f"crab_{name}_gensim_rawsim_hlt2024.py"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Submit combined GEN-SIM->RAWSIM+HLT2024 CRAB jobs (single task, no intermediate dataset) for DQCD Run3 2024 production.",
    ...
    p.add_argument(
        "--work-area", type=str, default="crab_gensim_rawsim_hlt2024",
    ...
    p.add_argument(
        "--lfn-base", type=str, default="/store/user/fernance/FinalScoutingProduction/GEN-SIM-RAW-HLT2024/",
    ...
    p.add_argument(
        "--tag", type=str, default="hlt_2024_v3",
        help="CRAB outputDatasetTag",
    )
    p.add_argument(
        "--max-memory-mb", type=int, default=3000,
        help="JobType.maxMemoryMB — the job runs GEN+SIM+DIGI+premix+full HLT menu+L1 emulator sequentially",
    )
    p.add_argument(
...
def main():
    ...
    print("Generating shared DIGI/RAW+HLT2024 cfg (reused for every sample):")
